...
const AUTHORIZATION = `Basic hd65erv7ddddsWEbfsckll3a5n`;
...
const updateFooterStatistic = () => {
  footerStatistic.remove();
  footerStatistic = new FooterStatistics(filmsModel.getFilms().length).getElement();
  render(footerStatisticsElement, footerStatistic, RenderPosition.BEFOREEND);
};
filmsModel.addObserver(updateFooterStatistic);
